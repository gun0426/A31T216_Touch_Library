...
static void app_crc_ccitt(void)
{
    uint32_t crc;
    ...
    HAL_CRC_Compute(0xFFFF, APP_CRC_DATA, APP_CRC_SIZE, &crc);
    LOG("HW = 0x%08x\n", crc);
     ...
     */
		crc = HAL_CRC_SW_Generate(0xFFFF, APP_CRC_DATA, APP_CRC_SIZE, cfg.poly, cfg.bit);
    LOG("SW = 0x%08x\n", crc);
}
