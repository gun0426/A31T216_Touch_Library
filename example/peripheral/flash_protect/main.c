...
static enum debug_cmd_status cmd_write_protect(int argc, char *argv[])
{
    uint8_t enable;
    ...
    HAL_FLASH_SetWriteProtect8KCodeArea(FLASH_PROT8K_CODE_SECTOR7, enable);

    return DEBUG_CMD_SUCCESS;
}

static enum debug_cmd_status cmd_read_protect(int argc, char *argv[])
{
    uint8_t lvl;
    ...
    lvl = (uint8_t)atoi(argv[1]);

    HAL_FLASH_SetReadProtectCodeArea((enum flash_read_prot_lvl)lvl);
...
static const struct debug_cmd test_commands[] =
{
    ...
    { "wprot", cmd_write_protect, "write/erase protect", "arg1=0:disable, 1:enable" },
    { "rprot", cmd_read_protect, "read protect", "arg1=lvl\n"
      " 0:unprotect\n"
      " 1:level 1\n" },
};
