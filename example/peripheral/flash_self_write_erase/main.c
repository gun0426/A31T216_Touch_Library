...
static enum debug_cmd_status cmd_erase(int argc, char *argv[])
{
    ...
    ret = HAL_FLASH_SelfErase(APP_FLASH_REGION_ADDR);

    LOG("Self Erase done %d\n", ret);
...
static enum debug_cmd_status cmd_write(int argc, char *argv[])
{
    ...
    ret = HAL_FLASH_SelfWrite(APP_FLASH_REGION_ADDR, write_buf, APP_FLASH_REGION_SIZE);

    LOG("Self Write done %d\n", ret);
