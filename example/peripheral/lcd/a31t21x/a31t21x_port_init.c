...
void Port_Configure_LCD(void)
{
    /* COM port setting - COM0 to COM2 */
    HAL_GPIO_SetFunction(PORTE, PIN7, Pn_MUX_AF0);	// COM0
    HAL_GPIO_SetMode(PORTE, PIN7, GPIO_MODE_FUNC);
    HAL_GPIO_SetFunction(PORTE, PIN6, Pn_MUX_AF0);	// COM1
    HAL_GPIO_SetMode(PORTE, PIN6, GPIO_MODE_FUNC);
    HAL_GPIO_SetFunction(PORTE, PIN5, Pn_MUX_AF0);	// COM2
    HAL_GPIO_SetMode(PORTE, PIN5, GPIO_MODE_FUNC);

		/* SEG port setting - SEG0 to SEG3 */
    HAL_GPIO_SetFunction(PORTE, PIN3, Pn_MUX_AF0);	// SEG0
    HAL_GPIO_SetMode(PORTE, PIN3, GPIO_MODE_FUNC);
    HAL_GPIO_SetFunction(PORTB, PIN11, Pn_MUX_AF0);	// SEG1
    HAL_GPIO_SetMode(PORTB, PIN11, GPIO_MODE_FUNC);
    HAL_GPIO_SetFunction(PORTB, PIN10, Pn_MUX_AF0);	// SEG2
    HAL_GPIO_SetMode(PORTB, PIN10, GPIO_MODE_FUNC);
    HAL_GPIO_SetFunction(PORTB, PIN9, Pn_MUX_AF0);	// SEG3
    HAL_GPIO_SetMode(PORTB, PIN9, GPIO_MODE_FUNC);
}
