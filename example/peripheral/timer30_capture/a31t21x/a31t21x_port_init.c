...
void Port_Configure_TIMER2n(void)
{
    ...
    HAL_GPIO_SetFunction(PORTC, PIN1, Pn_MUX_AF1);	// T21O
    HAL_GPIO_SetMode(PORTC, PIN1, GPIO_MODE_FUNC);
}

void Port_Configure_TIMER30(void)
{
    /* Configure TIMER30 pin to be used */
    HAL_GPIO_SetFunction(PORTF, PIN3, Pn_MUX_AF2);	// T30C
    HAL_GPIO_SetMode(PORTF, PIN3, GPIO_MODE_FUNC);
}
